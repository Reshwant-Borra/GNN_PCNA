...
def write_protein_doc(r):
    ...
    lines += [
        ...
        f"| File | Description |",
        f"|------|-------------|",
        f"| `results/per_structure/{pdb}/scores.csv` | Per-residue pocket scores |",
        f"| `results/per_structure/{pdb}/clusters.csv` | DBSCAN cluster assignments |",
        f"| `results/per_structure/{pdb}/report.txt` | Full text analysis report |",
        f"| `results/per_structure/{pdb}/summary.json` | Machine-readable summary |",
        f"",
        f"---",
        f"*GNN-PCNA v2 | Dual-branch GATv2Conv | Checkpoint: `checkpoints/pcna/best_pcna.ckpt`*",
    ]
