...
def test_inference_threshold_is_explicitly_configurable():
    src = (REPO / "scripts" / "run_v3_inference.py").read_text(encoding="utf-8")
    assert '"--threshold"' in src or "'--threshold'" in src
    assert "THRESHOLD" in src
    assert "high_mask = scores > threshold" in src


def test_seed_stability_can_select_validation_mcc_threshold():
    sys.path.insert(0, str(MD / "gnn_pocket_search"))
    import seed_stability as ss

    rows = [
        {"chain": "B", "resid": "1", "score": "0.90"},
        {"chain": "B", "resid": "2", "score": "0.80"},
        {"chain": "B", "resid": "3", "score": "0.20"},
        {"chain": "B", "resid": "4", "score": "0.10"},
    ]
    p = Path("threshold_fixture.csv")
    try:
        with p.open("w", newline="", encoding="utf-8") as fh:
            w = csv.DictWriter(fh, fieldnames=["chain", "resid", "score"])
            w.writeheader()
            w.writerows(rows)
        picked = ss.select_validation_mcc_threshold(p, "B", {1, 2})
    finally:
        if p.exists():
            p.unlink()
    assert picked["threshold"] == 0.5
    assert picked["mcc"] == 1.0
    assert picked["validation_chain"] == "B"
