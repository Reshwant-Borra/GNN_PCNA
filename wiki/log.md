...
## 2026-05-29 — GATE 4 approved: spatial-only model frozen

**Decision:** Reshwant approved GATE 4. Frozen checkpoint: checkpoints/phase3/spatial_only_fold1_seed1_best.pt (val macro-AUPRC 0.2047, fold=1 seed=1). Sequential edges permanently removed from primary model architecture.
**Evidence:** Option B retrain: spatial-only 0.1897 +/- 0.0091 vs full 0.1876 +/- 0.0113 across 12 runs; replicated across two independent training protocols (GATE 3 max_epochs=100 and this retrain max_epochs=200).
**Record:** eports/phase3/model_freeze_gate4_20260529.md (decision_id: phase3_model_freeze_20260529).
**Evidence status:** computational, validation only at time of freeze.

## 2026-05-29 — GATE 5 cleared: test-set evaluation complete

**Decision:** One-shot test evaluation run on frozen checkpoint. Test set will not be re-evaluated.
**Results:** macro-AUPRC = 0.2034 [0.1825, 0.2275] 95% CI. micro-AUPRC = 0.0973. macro-AUROC = 0.6902 [0.6652, 0.7143]. Top-20 recovery = 0.2179. 214 structures, 177 with valid AUPRC.
**Note on macro/micro gap:** micro-AUPRC (0.0973) substantially below macro (0.2034). This reflects pooled-residue imbalance and/or a few large proteins with many negatives diluting the micro signal. Does not indicate a bug; macro is the pre-specified primary metric.
